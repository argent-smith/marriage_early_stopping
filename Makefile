...
.PHONY: help setup setup-ocaml setup-python \
	demo demo-ocaml demo-python \
...
setup: setup-ocaml setup-python ## Настроить обе песочницы (opam switch + uv venv)
...
demo: demo-ocaml demo-python ## Прогнать все шесть вариантов подряд
...
test: test-ocaml test-python ## Прогнать тесты обоих субпроектов
...
lint: ## ruff check + ruff format --check для python/
	cd python && uv run ruff check .
	cd python && uv run ruff format --check .

fmt: ## ruff format (кроме marriage_early_stopping.py — транскрипция 1:1)
