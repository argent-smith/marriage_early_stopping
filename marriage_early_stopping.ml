...
(* marriage_early_stopping.ml — версия на Core.
   ...
   интеграция с Core и монадическая оптимизация в двух точках, где
   она реально что-то упрощает:

   1. Tracker.create теперь может завершиться ошибкой (patience <= 0)
      и возвращает 'a t Or_error.t. Точка входа (`run`) пробрасывает
      эту ошибку через монаду Or_error — инфиксными операторами
      (>>=), (>>|) из Base.Monad, а не через ppx-сахар let%bind.

   2. Tracker.reset и check_atmosphere используют (>>|) из монады
...
module Tracker = struct
  ...
  let create ?(patience = 3) () : 'a t Or_error.t =
    if patience <= 0 then
      (* [%string ...] — интерполирующий ppx (ppx_string, часть
         ...
         Printf. error_string, а не error_s/[%message]: сообщение
      ...
      Ok { patience; overfit_counter = 0; best_loss = Float.infinity; checkpoint = None }

  let is_new_best t ~loss = Float.(loss < t.best_loss)
  ...
  let should_reset t = t.overfit_counter >= t.patience
  ...
  let reset t : 'a t * 'a option =
    Option.(t.checkpoint >>| fun state -> ({ t with overfit_counter = 0 }, Some state))
    |> Option.value ~default:(t, None)
end
...
(** Один шаг наблюдения — аналог check_atmosphere из оригинала.
    Событие сброса тоже собирается инфиксной композицией: (>>|)
    переносит "restored" в пару (state, Reset_triggered), не трогая
    None; Option.value подставляет (current_state, Reset_skipped),
    если восстанавливать было нечего. *)
let check_atmosphere (type a) (t : a Tracker.t) ~(daily_loss : float)
    ~(current_state : a) : a Tracker.t * a * Event.t list =
  if Tracker.is_new_best t ~loss:daily_loss then
    ...
    in
    if not (Tracker.should_reset t') then (t', current_state, [ overfit_event ])
    else
      let t'', restored = Tracker.reset t' in
      let current_state', reset_event =
        Option.(restored >>| fun state -> (state, Event.Reset_triggered))
        |> Option.value ~default:(current_state, Event.Reset_skipped)
      in
